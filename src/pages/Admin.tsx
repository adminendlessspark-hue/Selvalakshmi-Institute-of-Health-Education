...
export function Admin() {
  const { 
    ...
    webinarVisible, updateWebinarVisible, loginStudent, restoreDefaultData
  } = useStore();
  ...
  const handleShareMeetLink = (apt: any) => {
    ...
    openExternalUrl(`https://wa.me/${apt.phone.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(message)}`);
  ...
  const [waInput, setWaInput] = useState<string>(whatsappNumber || "");
  const [ytInput, setYtInput] = useState<string>(youtubeUrl || "");
  const [igInput, setIgInput] = useState<string>(instagramUrl || "");
  const [fbInput, setFbInput] = useState<string>(facebookUrl || "");
  const [stInput, setStInput] = useState<string>(shareTemplate || "");
  ...
  React.useEffect(() => {
    ...
    setStInput(shareTemplate || "");
  }, [whatsappNumber, youtubeUrl, instagramUrl, facebookUrl, shareTemplate]);
  ...
  React.useEffect(() => {
    ...
    if (course) {
      ...
      if (templateLanguage === "en") {
        ...
        const phoneNo = whatsappNumber ? whatsappNumber : "+91 80728 87131";
        const socialLinksEn = [
          ...
          <div className="flex-1">
            ...
            {activeTab === "marketing" && (
              <div className="space-y-6">
                    ...
                    <div className="flex justify-start">
                      <button 
                        ...
                        onClick={async () => {
                          ...
                          try {
                            await updateSocialLinks(waInput.trim(), ytInput.trim(), igInput.trim(), fbInput.trim(), stInput.trim());
                            alert('Social links updated successfully!');
                          } catch (err) {
                            console.error('Error saving social links:', err);
                            alert('Failed to save social links. Please check your connection and try again.');
                          } finally {
                            setIsSavingSocial(false);
                          }
                        }}
                        className="bg-sage-600 text-white px-4 py-2 rounded-md hover:bg-sage-700 transition shadow-sm text-sm disabled:opacity-50"
                      >
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-amber-200 bg-amber-50/20 max-w-2xl">
                  <h3 className="font-bold text-slate-900 mb-2 flex items-center gap-2">
                    🔄 Restore Default Course Database & Student Testimonials (Videos & Audios)
                  </h3>
                  <p className="text-sm text-slate-600 mb-4">
                    If any course details, student testimonials (videos & audios), or default social media URLs are missing, click below to re-sync and restore default courses, testimonials, and social media links in Firebase.
                  </p>
                  <button 
                    disabled={isRestoringData}
                    onClick={async () => {
                      if (!confirm("Are you sure you want to restore default course database, student testimonials (videos & audios), and social links?")) return;
                      setIsRestoringData(true);
                      try {
                        await restoreDefaultData();
                        alert("Default course database, student testimonials (videos & audios), and social links restored successfully!");
                      } catch (err) {
                        console.error("Error restoring default data:", err);
                        alert("Failed to restore default data. Please check your connection and try again.");
                      } finally {
                        setIsRestoringData(false);
                      }
                    }}
                    className="bg-amber-600 text-white px-4 py-2 rounded-md hover:bg-amber-700 transition shadow-sm text-sm disabled:opacity-50 font-medium"
                  >
                    {isRestoringData ? "Restoring..." : "Restore Default Courses, Testimonials & Social Links"}
                  </button>
                </div>
                    ...
                    <div className="bg-sage-50/50 p-3 rounded-lg border border-sage-100/80">
                     ...
                     <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button 
                          type="button"
                          onClick={() => shareToWhatsApp(shareTemplateText)}
                          className="flex-1 min-w-[150px] flex items-center justify-center gap-2 bg-emerald-600 text-white hover:bg-emerald-700 px-3 py-2 rounded-lg transition font-bold text-xs shadow-sm cursor-pointer"
                        >
